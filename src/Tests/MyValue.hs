...
myValueTests, chooseMyValueTests, fromTests, toTests :: [Test]
myValueTests =
...
parseIntegersTest, parseStringsTest, parseBoolTest :: Test
parseIntegersTest =
  TestCase $
  assertEqual "for (parseIntegers \"{1,2}\")" "[1,2]" $ parseIntegers "{1,2}"

parseStringsTest =
  TestCase $
  assertEqual "for (parseStrings \"[k1,k2]\")" "[\"k1\",\"k2\"]" $
  parseStrings "[k1,k2]"

parseBoolTest =
  TestCase $ assertEqual "for (parseBool \"false\")" "False" $ parseBool "false"

chooseMyValueTests =
  [ TestLabel "chooseIntegersMyValueTest" chooseIntegersMyValueTest
  , TestLabel "chooseStringsMyValueTest" chooseStringsMyValueTest
  , TestLabel "chooseBoolMyValueTest" chooseBoolMyValueTest
  , TestLabel "chooseDateMyValueTest" chooseDateMyValueTest
  , TestLabel "chooseNextvalMyValueTest" chooseNextvalMyValueTest
  , TestLabel "chooseIntegerMyValueTest" chooseIntegerMyValueTest
  , TestLabel "chooseStringMyValueTest" chooseStringMyValueTest
  , TestLabel "chooseEmptyMyValueTest" chooseEmptyMyValueTest
  ]

chooseIntegersMyValueTest, chooseStringsMyValueTest, chooseBoolMyValueTest, chooseDateMyValueTest, chooseNextvalMyValueTest, chooseIntegerMyValueTest, chooseStringMyValueTest, chooseEmptyMyValueTest ::
     Test
chooseIntegersMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"[1,2]\" \"[1,2]\")" (MyIntegers [1, 2]) $
  chooseMyValue "[1,2]" "[1,2]"

chooseStringsMyValueTest =
  TestCase $
  assertEqual
    "for (chooseMyValue \"[K1,K2]\" \"[k1,k2]\")"
    (MyStrings ["k1", "k2"]) $
  chooseMyValue "[K1,K2]" "[k1,k2]"

chooseBoolMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"FALSE\" \"false\")" (MyBool False) $
  chooseMyValue "FALSE" "false"

chooseDateMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"2020-02-02\")" (MyDate "2020-02-02") $
  chooseMyValue "2020-02-02" "2020-02-02"

chooseNextvalMyValueTest =
  TestCase $
  assertEqual
    "for (chooseMyValue \"NEXTVAL(PERSON_ID_SEQ)\" \"nextval(person_id_seq)\")"
    (MyNextval "nextval(person_id_seq)") $
  chooseMyValue "NEXTVAL(PERSON_ID_SEQ)" "nextval(person_id_seq)"

chooseIntegerMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"1\" \"1\")" (MyInteger 1) $
  chooseMyValue "1" "1"

chooseStringMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"KEK\" \"kek\")" (MyString "kek") $
  chooseMyValue "KEK" "kek"

chooseEmptyMyValueTest =
  TestCase $
  assertEqual "for (chooseMyValue \"\" \"\")" MyEmpty $ chooseMyValue "" ""
