...
essenceParseTests :: [Test]
essenceParseTests =
  [ TestLabel "parsePsqlTest" parsePsqlTest
  , TestLabel "parsePsqlExprTest" parsePsqlExprTest
  ] <>
  parsePsqlExprTests <>
  parseTypeTests <> parseValueTests <> parseConstraintTests

parsePsqlTest, parsePsqlExprTest :: Test
parsePsqlTest =
  TestCase $
  assertEqual "for (parsePsql \"INTEGER\")" ("type", "int") $
  parsePsql "INTEGER"

parsePsqlExprTest =
  TestCase $
  assertEqual
    "for (runParser parsePsqlExpr \"\" \"\" \"INTEGER\")"
    (Right ("type", "int")) $
  runParser parsePsqlExpr "" "" "INTEGER"

parsePsqlExprTests :: [Test]
parsePsqlExprTests =
...
parseTypeTest, parseValueTest, parseRelationsTest, parseConstraintTest :: Test
parseTypeTest =
  TestCase $
  assertEqual
    "for (runParser parseType \"\" \"\" \"DATE\")"
    (Right ("type", "date")) $
...
parseValueTest =
  TestCase $
  assertEqual
    "for (runParser parseValue \"\" \"\" \"NOT NULL\")"
...
parseRelationsTest =
  TestCase $
  assertEqual
    "for (runParser parseRelations \"\" \"\" \"REFERENCES person (id)\")"
...
parseConstraintTest =
  TestCase $
  assertEqual
    "for (runParser parseConstraint \"\" \"\" \"PRIMARY KEY\")"
...
parseDateTest, parseBoolTest, parseUuidTest, parseIntTest, parseIntArrTest, parseStrTest, parseStrArrTest ::
     Test
parseDateTest =
  TestCase $
  assertEqual "for (runParser parseDate \"\" \"\" )" (Right "date") $
  runParser parseDate "" "" "DATE"

parseBoolTest =
  TestCase $
  assertEqual "for (runParser parseBool \"\" \"\" \"BOOLEAN\")" (Right "bool") $
  runParser parseBool "" "" "BOOLEAN"

parseUuidTest =
  TestCase $
  assertEqual "for (runParser parseUuid \"\" \"\" \"UUID\")" (Right "uuid") $
  runParser parseUuid "" "" "UUID"

parseIntTest =
  TestCase $
  assertEqual "for (runParser parseInt \"\" \"\" \"INTEGER\")" (Right "int") $
  runParser parseInt "" "" "INTEGER"

parseIntArrTest =
  TestCase $
  assertEqual
    "for (runParser parseIntArr \"\" \"\" \"INTEGER[]\")"
    (Right "array int") $
...
parseStrTest =
  TestCase $
  assertEqual
    "for (runParser parseStr \"\" \"\" \"VARCHAR(12)\")"
    (Right "string") $
...
parseStrArrTest =
  TestCase $
  assertEqual
    "for (runParser parseStrArr \"\" \"\" \"VARCHAR(12)[]\")"
    (Right "array string") $
...
parseNotNullTest, parseDefaultTest :: Test
parseNotNullTest =
  TestCase $
  assertEqual
    "for (runParser parseNotNull \"\" \"\" \"NOT NULL\")"
    (Right "not null") $
...
parseDefaultTest =
  TestCase $
  assertEqual
    "for (runParser parseDefault \"\" \"\" \"DEFAULT\")"
    (Right "null") $
  runParser parseDefault "" "" "DEFAULT"

parseConstraintTests :: [Test]
...
parsePrimaryKeyTest, parseUniqueTest, parseOnActionTest :: Test
parsePrimaryKeyTest =
  TestCase $
  assertEqual
    "for (runParser parsePrimaryKey \"\" \"\" \"PRIMARY KEY\")"
    (Right "primary key") $
...
parseUniqueTest =
  TestCase $
  assertEqual
    "for (runParser parseUnique \"\" \"\" \"UNIQUE\")"
    (Right "unique") $
